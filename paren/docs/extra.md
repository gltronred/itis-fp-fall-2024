...
# json + api на типах + htmx = веб-интерпретатор
...
Добавляется несколько новых выражений:
- `(thread-start! t)` - запускает новый поток `t`;
- `(make-thread f)` - создаёт поток, который будет выполнять `f`, функцию от 0 аргументов;
- `(thread-join! ...)` - дожидается завершения всех перечисленных потоков;
- `(make-shared-queue)` - создаёт очередь, доступную потокам;
- `(shared-queue-get! q)` - вытаскивает значение из очереди `q`;
- `(shared-queue-put! q v)` - кладёт значение `v` в очередь `q`.

# Опциональная строгая система типов

Добавляется новая форма:
- `(: var type)`, где `var` - имя, связанное с выражением, а `type` - тип.

Типы конструируются из базовых:
- `Number`
- `Bool`
- `Symbol`
- ...
и при помощи конструкторов
- `->` (задающих функции)
- `list` (задающих списки)

Например: `(: fib (-> Number Number))` или `(: + (-> Number Number Number))`

Система типов может дополняться и модифицироваться по необходимости.

# Проверяемая перед запуском

Система типов, определённая в предыдущем разделе, проверяется перед запуском программы.

Должно быть два режима: в одном, если не удаётся вывести тип выражения, программа выдаёт предупреждение, в другом - ошибку.
